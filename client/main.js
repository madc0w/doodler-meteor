...
const isContourDrawn = new ReactiveVar(false);
...
Template.main.helpers({
	...
	isContourDrawn: function () {
		return isContourDrawn.get();
	},
	...
	'click #draw-contours-button': function (e) {
		...
		Meteor.setTimeout(() => {
			...
			for (let i in edgePoints) {
				...
				if (isFuzzy) {
					const grd = outputContext.createRadialGradient(
						point[0],
						point[1],
						0,
						point[0],
			...
			if (edgeAlgorithm == 'none') {
				...
				for (let x = 1; x < outputCanvas.width - 1; x++) {
					for (y = 1; y < outputCanvas.height - 1; y++) {
						if (
							isWhite(getPixel(imageData, x, y)) &&
	...
	Meteor.setTimeout(() => {
		...
		Meteor.setTimeout(() => {
			...
			if (edgeDetectionType.get()) {
				$('#edge-detection-type-' + edgeDetectionType.get()).prop(
					'checked',
					true
				);
			}
...
function copyToPreview() {
	previewContext.drawImage(
		...
		0,
		previewCanvas.width,
		(img.height * previewCanvas.width) / img.width
	);
}
